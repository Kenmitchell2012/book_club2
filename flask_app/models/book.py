...
class Book:
    ...
    # validate the book update
    @staticmethod
    def validate_book_update(form_data):
        is_valid = True
        if len(form_data['title']) ==0:
            flash("Title is required!", 'book_update')
            is_valid = False
        if len(form_data['description']) < 2:
            flash("Descrption must be at least 2 characters long!", 'book_update')
            is_valid = False
        return is_valid
    ...
    def save_book(cls, data):
        ...
        query = """
                INSERT INTO books (title, description, user_id, creator)
                VALUES (%(title)s, %(description)s, %(user_id)s, %(creator)s);
                """
